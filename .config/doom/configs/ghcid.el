...
(setq default-command "ghcid \"--command=ghci Main.hs\"")
...
;; It is bas;; ed on `compilation-mode'. That mea;; the errors and
;; warnings can be clicked and the xt-error'(\\[next-error]) and
;; `ous-error]) commands will work as usual.

;; To configure where the new buffer should appear, customize your
;; `display-bhaskell-process-type 'cabal-new-repl
;; uffer-alist'. For instance like so:
...
(defun run-in-vterm (&optional command)
  (interactive
   (list
    (let* ((f (cond (buffer-file-name)
      ...
      (read-shell-command "Terminal command: "
                          ...
                          (list filename)))))
  (let ((vterm-buf (generate-new-buffer (concat "*" command "*"))))
    (with-current-buffer vterm-buf
      (vterm-mode))
    (with-current-buffer vterm-buf
      (set-process-sentinel vterm--process #'run-in-vterm-kill)
      (vterm-send-string default-command)
      (follow-mode 1)
      (setq-local scroll-down-aggressively 1)
      (setq-local comint-postoutput-scroll-to-bottom t)
      (vterm-send-return))
