...
cmd("augroup luis")
cmd("  au!")
cmd([[au ColorScheme * :lua require'status_custom'.create_highlights()]])
cmd("augroup END")
...
local function make_active_status_line()
	...
	local status_line = string.format("%%#%s# %s %%#StatusLineBg# ", hl, get_file_name(vim.bo.mod))
	status_line = status_line .. "%="
	status_line = status_line
		.. " %#StatusLineDeli#L:%#StatusLineColNb#%l %#StatusLineDeli#C:%#StatusLineLineNb#%v%< %#StatusLineDeli#(%#StatusLinePercent#%p%% %#StatusLineCount#%LL%#StatusLineDeli#) "
...
local function make_inactive_status_line()
	...
	local status_line = string.format(" %s %%#%s#", get_file_name(), hlend)
...
local function create_au()
	cmd("au BufEnter * lua require'status_custom'.make_status_line()")
end
